...
class FrameConfiguracao:
    ...
    def criar_widgets(self):
        # Tipo de lista
        ttk.Label(self.frame, text="Tipo de Lista:").grid(row=0, column=0, sticky="w", padx=5, pady=5)
        ...
        ttk.Radiobutton(opcoes_frame, text="Crescente", variable=self.tipo_lista, 
                       value="crescente").pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(opcoes_frame, text="Decrescente", variable=self.tipo_lista, 
                       value="decrescente").pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(opcoes_frame, text="Aleatória", variable=self.tipo_lista, 
                       value="aleatoria").pack(side=tk.LEFT, padx=5)
        
        # Tamanho da lista
        ttk.Label(self.frame, text="Tamanho da Lista:").grid(row=1, column=0, sticky="w", padx=5, pady=5)
        self.tamanho_var = tk.StringVar(value="100")
        ...
        self.entrada_custom = tk.Text(self.frame, height=3, width=50)
...
class JanelaGrafico:
    ...
    def criar_grafico_comparacao(self):
        ...
        max_ops = max(max(bubble_ops), max(insertion_ops), max(merge_ops)) if (bubble_ops and insertion_ops and merge_ops) else 1
